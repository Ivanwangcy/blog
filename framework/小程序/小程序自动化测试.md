...
### Tiga 常用的功能 API
- 跳转到指定页面：goto('/pages/send/sender')
- 截取屏幕快照：page.screenshot(options?)
- 获取页面元素：page.$(selector, options?)
- 模拟用户点击事件：elm.trigger(type, ...args?)
- 测试用例编写：describe('京东快递小程序自动化测试-首页渲染', function () { // 用例的内容 })
