...
首先，代码组织复杂，开发页面需要 4 个文件，包括页面模板，页面配置，页面样式，以及页面 JS 代码。我们在开发一个页面时需要在 4 个文件里来回切换，这样就显得有点复杂。当我们的应用变得越来越庞大的时候这个复杂度也会逐渐增加。

其次是规范不统一，在小程序开发过程中规范不统一是非常常见的。也是小程序诞生以来被人们诟病的一点。

还有一个是小程序孱弱的字符串模板。小程序的字符串模板是参考 Vue 的，但是并没有像 vue 那样提供非常丰富的功能。这就导致了小程序的模板写起来非常痛苦。字符串模板是不支持使用 ESLint 来及时发现错误的，这样子的话对我们开发也会造成一些影响。
...
其次就是Taro 提供了非常智能的代码提示，为我们的编码提供了非常流畅的开发体验，

最后就是Taro 提供了一个健全的代码错误检查机制，能够帮助我们及时的发现我们的代码错误，

以上只是提到了一些 Taro 非常核心，非常重要的特性。

还有很多其它特性没有在这里列出来。它最大的亮点就是，一次编写，多处运行。

那么在了解完 Taro 的特性以后，我们来看一下如何使用 Taro 来开发一个多端应用。

首先 Taro 也是一个 基于 Node 的开发框架，所以你必须要有一个比较新的 Node 环境。我们需要一个 Taro 的 cli 工具，然后将得到一个完整的 Taro 命令。
...
刚刚演示的使用 taro 如何开发应用，接下来我来介绍一下 Taro 的设计思想，掌握了Taro的设计思想有助于实际项目开发。那么 Taro 背后的设计思想是怎样的呢，

首先刚刚提到过了，Taro 采用了 React 语法来作为开发规范，用 React 开发方式带来了一系列的好处：
...
那么如何去实现多端应用呢？Taro 的目标是：

将 React 语法代码，通过 Taro 编译之后生成多端运行的代码。这里它就会涉及一个编译原理的概念。
...
左边是一个 Taro 组件代码；
右边是一个 原生小程序代码；
可以看出这两套代码的风格是完全不一样的，在 Taro 里面去创建类，创建组件它是以类继承的方式来进行的，而在小程序里面呢，就是以函数工厂的方式进行的。它们的生命周期方法也是完全不一样的；还有就是它的事件绑定不一样；那么我们要做的就是将Taro 这个的代码转换成小程序可以运行的代码。

而除此之外，在模板这一层 Taro 使用的是 JSX 语法，而小程序使用的是 WXML 字符串模板。

可以看到这两个模板做的是完全一样的事。通过循环来输出一些东西。在 JSX 中是通过 js 的 map 方法做循环遍历，而在小程序中是通过 wx-for 这样一个语法来做循环的遍历。

Taro 要做的便是将 JSX 编译成小程序的字符串模板。

刚刚也提到过，我们要达到这个的目的需要用到一个编译原理的思想，

所谓的编译原理就是将你输入的源代码，进行分词处理，然后词法分析得到代码字符串，再进行语法分析构建语法树，
再对语法树进行语义分析来检查程序正确性，最后经过一个转换操作，将语法树转换成目标代码。

这是一个完整的编译过程，听起来比较简单，但是操作起来确是非常繁琐，非常复杂的。

刚刚我们已经把代码转换部分搞定了，接下来就是如何实现多端应用，要进行多端适配方面的工作。

首先，通过代码转换操作，使代码可以在不同的平台上运行。
其次是进行运行时适配，使代码在不同的平台上有相同的表现；

我们来看看多端上的差异：

生命周期，组件 APi，
组件的差异，小程序中提供了很多基础组件构建应用比如：view, text, input 等等，在 H5 端 它也有一些标签来构建，看到出有些标签是做一样的事情。但是它们在不同端之间的用法是完全不一样的，这就是多端差异的一部分。

还有 API 方面，小程序里面是提供了很多 API, 比如网络请求等等，来帮你构建应用，在 H5 端也提供了一些 api，有些看似相同但用法完全不同。

根据以上这个不同端上的差异，Taro 又使怎样实现多端适配的呢？

 Taro 里面会定义一些基础标准，这个基础标准包括了：基础组件库，Taro bootstrap (这样一个运行时框架), Taro 标准的 API，
 

 通过这3个部分构成了一个 Taro 基础标准。而 Taro 需要做的就是去实现这些基础标准。比如 组件库在各平台都有对应的实现，API 和 bootstrap 也是一样。不同端不同的实现。通过这些操作我们即可实现多端适配。

Taro 的一个完整的架构就是这样子的。
首先通过编译转换代码，使代码可以运行在多个平台；
运行时适配，使得不同平台代码有相同表现。
